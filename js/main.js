...
(function () {
  ...
  function initNav() {
    ...
    document.querySelectorAll(".nav a").forEach(function (link) {
      link.addEventListener("click", function () {
        if (window.innerWidth <= 1080 && !link.parentElement.classList.contains("dropdown")) closeNav();
      });
    });

    document.addEventListener("keydown", function (event) {
      if (event.key === "Escape" && nav && nav.classList.contains("open")) closeNav();
    });
  ...
  function initNavbar() {
    ...
    const pagePath = window.location.pathname.split("/").filter(Boolean);
    if (pagePath.length && pagePath[pagePath.length - 1].indexOf(".") !== -1) pagePath.pop();
    const prefix = pagePath.map(function () { return ".."; }).join("/");
    const navbarPath = (prefix ? prefix + "/" : "") + "navbar.html";
      ...
      .then(function (html) {
        ...
        template.content.querySelectorAll("[href], [src]").forEach(function (element) {
          ["href", "src"].forEach(function (attribute) {
            ...
            element.setAttribute(attribute, rootPrefix + value);
          });
        });
        ...
        mount.querySelectorAll(".nav a[href]").forEach(function (link) {
